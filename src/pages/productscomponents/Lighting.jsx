...
import { solarLightingProducts } from "../../utils/solarproducts";

function Lighting({ search }) {
  const lighting = solarLightingProducts.filter(
    (light) =>
      light.type.toLowerCase() === "lighting" &&
      light.name.toLowerCase().includes(search.toLowerCase())
  );
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {lighting.length > 0 ? (
        lighting.map((light) => light.length)
      ) : (
        <p className="text-gray-500 col-span-full text-center mt-10">
          No Lighting found matching "{search}"
        </p>
      )}
      {lighting.map((light, index) => (
        <motion.div
          ...
          <div className="relative border border-gray-300/20 dark:border-gray-600 min-h-[200px] flex items-center justify-center rounded-[18px] shadow-md dark:shadow-none overflow-hidden">
            <img
              src={"/lightings.webp" || light.image}
              alt={light.name}
              loading="lazy"
          ...
          <div className="flex-1 flex flex-col gap-2">
            <p className="text-sm text-gray-500 font-medium">{light.type}</p>
            ...
            <p className="text-xs text-gray-400 mt-1">📍 {light.location}</p>
            ...
            <div className="mt-3 text-sm text-gray-600 dark:text-gray-300">
              <p>
                ...
                {light.details.brightness || "-"}
              </p>
              <p>
                <span className="font-semibold">Runtime:</span>{" "}
                {light.details.runtime || "-"}
              </p>
              <p>
                {light.details.waterproof && (
                  <>
              ...
              <p>
                {light.details.bulbs && (
                  <>
                    <span className="font-semibold">Bulbs:</span>{" "}
                    {light.details.bulbs}
                  </>
                )}
              </p>
              <p>
                {light.details.poleHeight && (
                  <>
                    <span className="font-semibold">Pole Height:</span>{" "}
                    {light.details.poleHeight}
                  </>
                )}
              </p>
