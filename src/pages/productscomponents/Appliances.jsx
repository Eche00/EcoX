...
import { solarAppliances } from "../../utils/solarproducts";

function Appliances({ search }) {
  const appliances = solarAppliances.filter(
    (product) =>
      product.type.toLowerCase() === "appliances" &&
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {appliances.length > 0 ? (
        appliances.map((appliance) => appliance.length)
      ) : (
        <p className="text-gray-500 col-span-full text-center mt-10">
          No Appliance found matching "{search}"
        </p>
      )}
      {appliances.map((appliance, index) => (
        <motion.div
          ...
          <div className="relative border border-gray-300/20 dark:border-gray-600 min-h-[200px] flex items-center justify-center rounded-[18px] shadow-md dark:shadow-none overflow-hidden">
            <img
              src={"/appliancees.webp" || appliance.image}
              alt={appliance.name}
              loading="lazy"
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            <div className="mt-3 text-sm text-gray-600 dark:text-gray-300">
              <p>
                ...
                {appliance.details.power || "-"}
              </p>
              <p>
                <span className="font-semibold">Flow Rate:</span>{" "}
                {appliance.details.flowRate || "-"}
              </p>
              <p>
                <span className="font-semibold">Head:</span>{" "}
                {appliance.details.head || "-"}
              </p>
              <p>
                <span className="font-semibold">Warranty:</span>{" "}
                {appliance.details.warranty || "-"}
              </p>
            </div>
