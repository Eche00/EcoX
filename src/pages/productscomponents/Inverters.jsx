...
import { solarInverters } from "../../utils/solarproducts";

function Inverters({ search }) {
  const inverters = solarInverters.filter(
    (inverter) =>
      inverter.type.toLowerCase() === "inverters" &&
      inverter.name.toLowerCase().includes(search.toLowerCase())
  );
  return (
    <main className="w-full py-10 px-4 sm:px-10 min-h-screen">
      {inverters.length > 0 ? (
        inverters.map((inverter) => inverter.length)
      ) : (
        <p className="text-gray-500 col-span-full text-center mt-10">
          No Inverter found matching "{search}"
        </p>
      )}
        ...
        {inverters.map((inverter, index) => (
          <motion.div
            ...
            <div className="relative border border-gray-300/20 dark:border-gray-600 min-h-[200px] flex items-center justify-center rounded-[18px] shadow-md dark:shadow-none overflow-hidden">
              <img
                src={"/inverters.webp" || inverter.image}
                alt={inverter.name}
                loading="lazy"
            ...
            <div className="flex-1 flex flex-col gap-2">
              ...
              <div className="mt-3 text-sm text-gray-600 dark:text-gray-300">
                <p>
                  ...
                  {inverter.details.type}
                </p>
                <p>
                  <span className="font-semibold">Output:</span>{" "}
                  {inverter.details.output}
                </p>
                <p>
                  <span className="font-semibold">Voltage:</span>{" "}
                  {inverter.details.voltage}
                </p>
                <p>
                  <span className="font-semibold">Warranty:</span>{" "}
                  {inverter.details.warranty}
                </p>
              </div>
